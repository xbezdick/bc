...
\section{Cairo}
Cairo je knižnica určená na kreslenie 2D vektorov napísaná v jazyku C a rovnako ako GTK+ je použiteľná v ostatných programovacích jazykoch za pomoci jazykových väzieb. Za pomoci caira je možno kresliť do viacerých výstupov ako napríklad PDF, PNG\footnote{Portable Network Graphics.}, GtkWindow. Model kreslenia pomocou knižnice cairo pozostáva z týchto definícií:
\begin{description}
\item[Context] reprezentuje kontext v ktorom chcem kresliť. To znamená, že popisuje čo, kam a ako.
...
\item[Source] reprezentuje zdroj pozostávajúci z farby, gradientu, obrázku alebo vzoru. V podobnej analógii ako v predošlom prípade reprezentuje farbu na štetci.
\item[Mask] reprezentuje šablónu, podľa ktorej sa aplikuje naša kresba.
\end{description}
Kreslenie pomocou cairo prebieha tak, že sa vytvorí kontext cairo\_t, nastaví sa mu povrch a nastavenia. V prípade tejto práce išlo o kontext s povrchom widgetu. Následne pomocou funkcií cairo\_set\_source\_rgb sa vyberie farba, cairo\_set\_line\_width určí šírku čiary, cairo\_move\_to presunie na východziu pozíciu, cairo\_line\_to určí vlastnosti čiary (cestu) a funkcia cairo\_stroke následne čiaru (cestu) nakreslí. Cairo ponúka mnoho ďalších funkcií --- napríklad na kreslenie obdĺžnikov ..., vyplnenie cesty ... --- bližšie popísaných v dokumentácii LINK
